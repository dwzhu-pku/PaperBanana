...
| WP-007/WP-105 | `EV-20260622-035` covers aggregate native/Python/Xcode gate plus Release build/install proof for current branch head `f360dc6d5ccd`; repeat remains required on any later product-code SHA or different frozen release-candidate SHA. |
| WP-106 | Approved live provider/fallback native E2E with non-private fixtures, spend limit, redacted request/metadata/provider-artifact review, and failure/recovery proof on the final candidate. `EV-20260622-038` covers no-spend native generation/refinement store artifact and provenance behavior only; `EV-20260622-039` covers mocked/no-live cancellation, timeout, and stale-run recovery only. |
| WP-109 | True install/upgrade/rollback proof and release-manifest consistency on the final frozen release SHA. `EV-20260622-036` adds a draft current-candidate manifest and contract coverage, and `EV-20260622-037` proves local app-bundle rollback preflight mechanics plus focused RunStore migration coverage, but neither proves distinct prior-version upgrade, full user-data migration, hosted rollback, or final release readiness. |
