...
| WP-106 | Search/filter and 10-of-10 selection-cap manual validation against real PaperBananaBench data; `EV-20260622-023` covers real-data availability, thumbnail rendering, missing-image warning, and one-selection UI state. |
| WP-106 | Durable selected-reference provenance inspection for real PaperBananaBench data through the native no-spend dry-run control or an approved Codex/provider run. |
