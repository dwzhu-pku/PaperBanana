...
| Latest WP-106 current-head real Codex compile-skip evidence head | `f963f4d77b022a324e4aa2b5e5896122320c177c` |
...
`a251dda11fa2`.
`EV-20260623-095` records repo-level current-Xcode compatibility on
`29901fa32d9a`: `script/check_native_xcode_contract.sh` now consumes the same
explicit `PAPERBANANA_EXPECTED_XCODE_BUILD='Build version 27A5209h'` override
as the baseline guard, the overridden native Xcode project contract passed, the
native source-control contract passed, repeated `xcodebuild test` passed with
167 tests, and the isolated Python 3.12 suite passed with 147 tests. The
aggregate `script/test_all.sh` command still failed before XCTest because the
global `codex-xcode27 host-audit` path keeps an internal `27A5194q` expected
...
`EV-20260623-097` refreshes that live-only XCTest on current branch head
`f963f4d77b022a324e4aa2b5e5896122320c177c`: the targeted Xcode command supplied
`OTHER_SWIFT_FLAGS='-D PAPERBANANA_ENABLE_REAL_CODEX_E2E_TESTS'`, deliberately
omitted `PAPERBANANA_REAL_CODEX_E2E=1`, wrote
`/tmp/PaperBanana-real-codex-skip-f963f4d.xcresult`, and `xcresulttool`
...
through `VO-16` route coverage, placeholder policy, stop conditions, no-live
and no-secret boundary, cleanup/environment/defect templates, and a CI-safe
...
- Full manual keyboard navigation and VoiceOver traversal across Settings,
  ...
  session.
  `EV-20260623-090` defines the remaining manual VoiceOver packet with final-SHA
  provenance, installed app hash capture, same-display/no-live preconditions,
  required artifact files, pass criteria, stop conditions, route checklist, and
  speech-output capture template.
  `EV-20260623-091` adds a separate artifact contract and checked templates for
