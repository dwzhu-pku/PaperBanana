...
- [x] Upload code for improving existing diagrams and plots based on style guideline.
...
- Manual PaperBananaBench diagram and plot example selection inside Prompt Studio for native prompt enrichment.
...
1. **Retriever Agent**: Identifies the most relevant reference examples from a curated collection to guide downstream agents
...
When `data/PaperBananaBench/diagram/ref.json` or `data/PaperBananaBench/plot/ref.json` is present, Prompt Studio can attach up to 10 manually selected examples for the active native generation task. The app records those examples in `request.json`, generated metadata, and the provider prompt. PaperBanana does not download or unpack the benchmark automatically.

Prompt Studio can also run the `Statistical Plot` task through the native provider path. Plot reference examples use `data/PaperBananaBench/plot/ref.json`; if that local dataset is missing, plot runs proceed without manual examples and record `reference_mode: none`.
...
- Choose `diagram` or `plot` as the output type.
...
**2. Refine Image Tab**:
- Upload a generated candidate or any figure or plot.
...
### Interactive Demo
- **Parallel Generation**: Generate up to 20 candidate figures or plots simultaneously
