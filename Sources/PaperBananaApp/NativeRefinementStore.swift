...
@MainActor
final class NativeRefinementStore: ObservableObject {
    ...
    @Published private(set) var sourceCopyURL: URL?
    ...
    func start(request: NativeRefinementRequest, onCompletion: @escaping @Sendable (URL) -> Void) {
        guard !isRunning else { return }
        let trimmedPrompt = request.prompt.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmedPrompt.isEmpty else {
            fail("Edit instructions are required.")
            return
        }

        let repoRoot = URL(fileURLWithPath: request.settings.repoPath, isDirectory: true)
        let outputDirectory = repoRoot.appendingPathComponent("results/native_refine", isDirectory: true)
        let runID = request.preflightRunID ?? Self.makeRunID(sourceURL: request.sourceURL)
        let runDirectory = outputDirectory.appendingPathComponent(runID, isDirectory: true)
        let sourceCopyURL = runDirectory.appendingPathComponent(Self.safeSourceFilename(request.sourceURL))
        ...
        let predictedOutputURL = runDirectory
            .appendingPathComponent("\(Self.safeFileStem(request.sourceURL.deletingPathExtension().lastPathComponent))_refined_\(request.resolution)")
        ...
        do {
            ...
            fail("Could not create durable refinement run record: \(error.localizedDescription)")
            return
        }
    ...
    func cancel() {
        let message = "Refinement cancelled by user."
        if let ledgerError = markActiveProviderCallTerminal(status: .cancelled, message: message) {
            let failureMessage = "Refinement cancelled locally, but PaperBanana could not record the provider call as cancelled: \(ledgerError.localizedDescription)"
    ...
    private func resetForRun(
        ...
        runState = .running
        progress = 0
        statusMessage = "Queued native refinement."
    ...
    private func localProviderClient(
        for executionMode: NativeRefinementExecutionMode,
        providerPlan: ImageProviderExecutionPlan
    ) -> (any ProviderClient)? {
        ...
        case .mockProviderValid:
            NativeLocalProviderClient(providerKind: providerPlan.provider, mode: .mockValid)
        case .mockProviderInvalidPayload:
            NativeLocalProviderClient(providerKind: providerPlan.provider, mode: .mockInvalidPayload)
        }
    }
    ...
    private func startNativeProviderExecution(
        ...
        guard let repoRoot = activeRepoRootURL,
              let sourceCopyURL else {
            fail("No active source image is available for native provider execution.")
            return
        }
    ...
    private func completeNativeProviderResponse(
        ...
        repoRoot: URL,
        providerPlan _: ImageProviderExecutionPlan,
        prompt: String,
        request: NativeRefinementRequest,
        onCompletion: @escaping @Sendable (URL) -> Void
    ) {
        ...
        do {
            let completion = try NativeProviderCompletionCoordinator.completeImageResponse(
                ...
                savingMessage: "Saving refined image.",
                successFallbackMessage: "Image refined successfully.",
                failureMessagePrefix: "Failed to save refined image",
                ...
                writeMetadata: {
                    try NativeRefinementMetadataWriter.write(
    ...
    private func rawPayloadURLDidFail(_ error: Error) {
        if let rawPayloadURL,
           FileManager.default.fileExists(atPath: rawPayloadURL.path) {
            let message = "Refined provider bytes were preserved as a raw recoverable payload, but PaperBanana could not normalize them into a PNG: \(error.localizedDescription)"
            sawTerminalEvent = true
        ...
        let message = "Failed to save refined image: \(error.localizedDescription)"
    ...
    private func prepareDurableRunRecord(request: NativeRefinementRequest, providerPlan: ImageProviderExecutionPlan, prompt: String) throws {
        ...
        let repoRoot = URL(fileURLWithPath: request.settings.repoPath, isDirectory: true)
        try NativeRefinementDurableRunWriter.prepare(
            request: request,
            providerPlan: providerPlan,
            prompt: prompt,
            runID: runID,
        ...
        appendLocalEvent(
            stage: "queued",
            progress: 0,
            message: "Created durable native refinement run record."
        )
    }

    private func failUnsupportedNativeProvider(_ providerPlan: ImageProviderExecutionPlan) {
        fail(
            "Native refinement does not support \(providerPlan.providerLabel) for \(providerPlan.backendModelValue). " +
    ...
    private func tickProgressClock() {
        ...
        if secondsSinceLastEvent >= hardTimeoutInterval {
            let message = "No provider progress for \(Self.formatDuration(secondsSinceLastEvent)); terminating local refinement process."
            if let ledgerError = markActiveProviderCallTerminal(status: .timedOut, message: message) {
                let failureMessage = "Refinement timed out locally, but PaperBanana could not record the provider timeout in the durable ledger: \(ledgerError.localizedDescription)"
                appendLocalEvent(stage: "failed", progress: progress, message: failureMessage)
                sawTerminalEvent = true
                providerTask?.cancel()
                providerTask = nil
